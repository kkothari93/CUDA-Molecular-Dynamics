...
#pragma unroll
		for (int i = 0; i < 3; i++){
			...
			if (fabs(del)> L / 2.0){ n = 1 + i; breachindex = index; }
	...
	if (index < N_ATOMS*N_ATOMS){
		...
		if (del <1.0f && del!=0.0f){ printf("distance breached at index %i\tatom 1: %i\tatom 2 : %i\tdistance: %f\n", index, iatom1, iatom2,del); }
	}
}
...
void create_dump(char *filename, float *data, int steps, int nparticles){
	...
	for (i = 0; i<steps; i++){
		...
		for (j = 0; j<nparticles; j++)
			fprintf(fp, "%i %i %f %f %f 0 0 0\n", j + 1, 1, data[stride + j], data[stride + j + N_ATOMS], data[stride + j + 2 * N_ATOMS]);
	}
	fclose(fp);
	printf("\n %sfile created \n", filename);
}
...
int main(){
	...
	for (int t = 0; t<NUM_STEPS; t++){
		...
		if (t % 1== 0){ printf("Potential energy at iteration [%i] is %1.10f \n", t, PEtrace[t]); }
