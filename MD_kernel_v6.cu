...
const int scheme = 0; // 0 for explicit, 1 for implicit
...
__global__ void newForceReduction(float *input, float *output, int startunit, int len)
{
	...
	__shared__ float partSum[N_ATOMS];
	...
	if (start + tx < len){
		partSum[tx] = input[start + tx];
		if (start + BLOCK_SIZE1 + tx < len) partSum[BLOCK_SIZE1 + tx] = input[start + BLOCK_SIZE1 + tx];
		else partSum[BLOCK_SIZE1 + tx] = 0;
	}
	else{
		partSum[tx] = 0;
		partSum[BLOCK_SIZE1 + tx] = 0;
	}

	if (N_ATOMS % 2)
	{
		if (start + 2 * BLOCK_SIZE1 + 1< len)
		{
			if (tx == 0) partSum[2 * BLOCK_SIZE1] = input[start + 2 * BLOCK_SIZE1];
		}
		else if (tx == 0) partSum[2 * BLOCK_SIZE1] = 0;
	}

	//Reduction Kernel for each dimension
	unsigned int stride;
	for (stride = BLOCK_SIZE1; stride > 0; stride = stride / 2){
		__syncthreads();
		if (tx < stride) partSum[tx] += partSum[tx + stride];
	}

	if (tx == 0){
		if (N_ATOMS % 2) { partSum[0] += partSum[2 * BLOCK_SIZE1]; }
		output[blockIdx.x] = partSum[0];
		//printf("Answer for bx(%i) %f \n", blockIdx.x, partSum[0]);
	}
}
...
int main(){
	...
	//	float a = 5.0f;
	//	printf("a before put in box %f\n", a);
	//	a = PutInBox2(a);
	//	printf("a after put in box %f\n", a);
	...
	for (int t = 0; t<NUM_STEPS; t++){
		...
		newForceReduction << <3 * N_ATOMS, BLOCK_SIZE1 >> >(d_ForceOut, d_ForceOutRed1, N_ATOMS, 3 * N_ATOMS*N_ATOMS);
